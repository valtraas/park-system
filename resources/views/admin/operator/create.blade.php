            ...
            <form action="{{ route('management.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="nama-lengkap text-center mb-3">
                    ...
                    <input type="text" name="nama"
                        class="block border border-slate-900 px-1 rounded-md h-8 @error('nama')  ring-2 ring-red-600 @enderror mx-auto" placeholder="Masukan nama lengkap" value="{{ old('nama') }}">
                    @error('nama')
                ...
                <div class="username text-center mb-3">
                    ...
                    <input type="text" name="username"
                        class="block border border-slate-900 px-1 rounded-md h-8 @error('username')  ring-2 ring-red-600 @enderror mx-auto" placeholder="Masukan Username" value="{{ old('username') }}">
                    @error('username')
                        <div class="text-red-600">
                            {{ $message }}
                        </div>
                ...
                <div class="kenaraan mb-3 text-center">
                    ...
                    <select id="Role" name="role_id"
                        ...
                        @foreach ($role->skip(1) as $item)
                            <option value="{{ $item->id }}" {{ ($item->id == old('role_id') ? 'selected' : '') }}>{{ $item->nama }}</option>
                        @endforeach
                    </select>
                    <div>
                        @error('role_id')
